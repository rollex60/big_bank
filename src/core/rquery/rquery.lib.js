...
class RQuery {
   ...
   * @param {string} selector - A CSS selector string to search for
   * within the selected element.
   ...
   * @param {string} selector - A CSS selector string to search for
   * within the selected element.
   * @returns {RQuery[]} An array of new RQuery instance for the
   * found elements 
   */
  findAll(selector) {
   ...
   * @param {HTMLElement} childElement - The new child element to
   * append.
   ...
   * chaining when setting HTML content, or the current inner HTML when
   * getting.
   */
  html(htmlContent) {
   ...
   * event type.
   * @param {string} eventType - The type of event to listener for (e.
   * g., 'click', 'input', etc.).
   * @param {function(Event): void} callback - The event listener
   * function to execute when the event is triggered. The function will
   * receive the event object as it's argument.
   * @returns {RQuery} The current RQuery instance for chaining.
   */
  on(eventType, callback) {
    if (typeof eventType !== 'string' || typeof callback !==
    'function') {
      throw new Error(
   ...
   * function will receive the event object at its argument.
   * @returns {RQuery} The current RQuery instance for chaining.
    */
   click(callback) {
    ...
    * @return {string|RQuery} - If newValue is provided, returns the
    ...
    * @param {object} [options.reset] - Optional attributes to set on
    * the input element.
    * @returns {RQuery} The current RQuery instance for chaining.
    */
   input({onInput, ...rest}) {
   ...
   /**
    * Set attribute and event listeners for a credit card input
    * element.
    * @returns {RQuery} The current RQuery instance for chaining.
    */
   creditCardInput() {
   ...
   /**
    * Hides the selected element by setting its display style to
    * 'none'.
    * @returns {RQuery} The current RQuery instance for chaining.
    */
   hide() {
   ...
   * @param {string | string[]} classesName - A single class name or
